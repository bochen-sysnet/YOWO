...
def evaluate_videoAP(gt_videos, all_boxes, CLASSES, bbx_pred_t, iou_thresh = 0.2, bTemporal = False, ref_frame_cnt = 20, prior_length = None):
    ...
    print_str += "Video mAP:{0:.3f},{1:.3f}\n".format(np.mean(ap_all), np.mean(ap_new_all))
    print_str += "Total loc time:{0:.3f},{1:.3f}\n".format(act_loc_t_old, act_loc_t_new)
    print_str += "EALR:{0:.4f},{1:.4f}\n".format(np.mean(ap_all)/act_loc_t_old, np.mean(ap_new_all)/act_loc_t_new)
