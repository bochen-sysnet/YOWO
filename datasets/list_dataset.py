...
class UCF_JHMDB_Dataset_codec(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.train: # For Training
            frame_idx, clip, label, bpp_est, loss = load_data_detection_from_cache(self.base_path, imgpath,  0, self.clip_duration, self.sampling_rate, self.cache, self.dataset)
    ...
    def preprocess(self, index, model_codec):
        ...
        if cur_video != self.prev_video or self.cache['max_idx'] != im_ind-2:
            # read raw video clip
            clip,misc = read_video_clip(self.base_path, imgpath,  0, self.clip_duration, self.sampling_rate, self.shape, self.dataset)
            # frame shape
