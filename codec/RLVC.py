...
class RLVC(nn.Module):
    def __init__(self):
        super(RLVC, self).__init__()
    ...
    def forward(self, Y0_com, Y1_raw, prior_latent, hidden, RPM_flag, use_psnr=True): 
        ...
        return Y1_com, hidden, prior_latent, bpp_est, bpp_act, metrics, loss

def PSNR(Y1_raw, Y1_com):
    train_mse = torch.mean(torch.pow(Y1_raw - Y1_com, 2))
    log10 = torch.log(torch.FloatTensor([10]))
